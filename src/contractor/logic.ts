...
export class CreateContractorLogic extends Logic {
    ...
    async execute(profileData: any) {
        ...
        let status = user.baseProfile.dwollaStatus;
        if (!user.baseProfile.dwollaUri) {
            user.baseProfile.merge(profileData);
            const customer = dwolla.customer.factory(profileData);
            customer.type = dwolla.customer.TYPE.Personal;
            user.baseProfile.dwollaUri = await this.dwollaClient.createCustomer(customer);
            const dwollaCustomer = await this.dwollaClient.getCustomer(user.baseProfile.dwollaUri);
...
export class UpdateContractorStatusLogic extends Logic {
    ...
    async execute(user: User, status: string, trx?: objection.Transaction): Promise<any> {
        ...
        if (user.baseProfile.dwollaStatus === status) return;

        user.baseProfile.dwollaStatus = status;
        user.tenantProfile.dwollaStatus = status;

        // TODO: update the profiles statuses using state machine
        user.tenantProfile.status = profiles.Statuses.bank;
        user.baseProfile.status = profiles.Statuses.bank;

        await objection.transaction(this.profileService.transaction(trx), async _trx => {
            await this.profileService.update(user.baseProfile, _trx);
