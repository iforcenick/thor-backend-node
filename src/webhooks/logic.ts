...
export class CustomerFundingSourceEventLogic extends Logic {
    ...
    async execute(event: IEvent): Promise<any> {
        const profile = await this.profileService.getByResourceLink(event['_links']['customer']['href']);
        if (!profile) {
            throw new Error(`${TAG} Could not find profile by dwollaUri ${event['_links']['customer']['href']}`);
        }

        const user = await this.userService.get(profile.userId);
        if (!user) {
            throw new Error(`${TAG} Could not find user by profile ${profile.id}`);
        }

        const fundingSource = await this.fundingSourceService.getByDwollaUri(event['_links']['resource']['href']);
        if (!fundingSource) {
            throw new Error(`${TAG} Could not find funding source by dwollaUri ${event['_links']['resource']['href']}`);
        }
...
export class CustomerTransferEventLogic extends Logic {
    ...
    async execute(event: IEvent): Promise<any> {
        const transaction = await this.transactionService.getByTransferDwollaUri(
            event['_links']['resource']['href'],
        );
        if (!transaction) {
            throw new Error(`${TAG} Could not find transaction by dwollaUri ${event['_links']['resource']['href']}`);
        }
        if (!transaction.status) {
            throw new Error(`${TAG} Invalid transaction`);
        }
        ...
        try {
            ...
            switch (event.topic) {
                case dwolla.event.TYPE.customerTransfer.created:
                    await logic.execute(transaction, Statuses.processing);
                    break;

                case dwolla.event.TYPE.customerBankTransfer.completed:
                    await logic.execute(transaction, Statuses.processed);
                    break;
                ...
                case dwolla.event.TYPE.customerTransfer.failed:
                case dwolla.event.TYPE.transfer.failed:
                    await logic.execute(transaction, Statuses.failed);
                    break;
...
export class CustomerEventLogic extends Logic {
    ...
    async execute(event: IEvent): Promise<any> {
        ...
        if (!user.isContractor()) {
            throw new Error(`${TAG} This user is not a contractor`);
        }
